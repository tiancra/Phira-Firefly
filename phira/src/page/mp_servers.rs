...
/// `host:port` with a nonzero port — the multiplayer client dials a TCP socket.
fn validate_address(address: &str) -> Result<()> {
    let Some((host, port)) = address.rsplit_once(':') else {
        anyhow::bail!("server address must be host:port");
    };
    if host.is_empty() {
        anyhow::bail!("server address must include a host");
    }
    match port.parse::<u16>() {
        Ok(port) if port != 0 => Ok(()),
        _ => anyhow::bail!("server address must include a nonzero port"),
    }
}
