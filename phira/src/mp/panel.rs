...
use prpr::{
    ...
    info::ChartInfo,
    scene::{mp_reset_result, mp_take_result, request_input, return_input, show_error, show_message, take_input, GameMode, NextScene},
    task::Task,
    time::TimeManager,
...
pub struct MPPanel {
    ...
    need_upload: bool,
    // 本局是否为本地谱面：进入 Playing 时 local_chart 会被清理，需单独记录以供游玩结束后的成绩上报选择渠道
    game_local_chart: bool,
    entered: bool,
...
impl MPPanel {
    ...
    pub fn update(&mut self, tm: &mut TimeManager) -> Result<()> {
        ...
        if let Some(client) = &self.client {
            ...
            if matches!(state, Some(RoomState::Playing)) {
                if !self.game_start_consumed {
                    ...
                    self.entered = false;
                    // 本地谱面在进入 Playing 后 local_chart 会被清理，这里单独记录本局类型，
                    // 否则游玩结束后会被误判为在线谱面从而走 abort() 分支。
                    self.game_local_chart = self.local_chart.is_some();
        ...
        if self.need_upload && self.entered {
            ...
            let is_local_chart = self.game_local_chart;
            // 单人房间中游玩期间一旦连接断开并重连，服务端可能已在 Playing 状态清理掉房间，
            // 此时再上报成绩会得到 "no room" 报错，因此若已不在房间则跳过成绩上报。
            let in_room = self.client.as_ref().is_some_and(|it| it.blocking_state().is_some());
            if in_room {
                let client = self.clone_client();
