
label = Ustawienia

general = Ogólne
audio = Dźwięk
chart = Beatmapa

item-lang = Język
item-offline = Tryb offline
item-offline-sub = Nie możesz wysłać swojego rekordu w trybie offline
item-lowq = Niska jakość
item-lowq-sub = Włącz tą opcje, jeśli UI się zacina
item-mp = Multiplayer
item-mp-sub = Włącz funkcję gry wieloosobowej
item-mp-addr = Serwer multiplayer
item-mp-addr-sub = Zarządzaj serwerami multiplayer
item-mp-addr-invalid = Nieprawidłowy adres serwera
mp-server-page = Serwery multiplayer
mp-server-none = Nie wybrano serwera
mp-server-current = Aktualny
mp-server-name-title = Nazwa serwera
mp-server-name-prompt = Wpisz notatkę dla tego serwera.
mp-server-name-empty = Nazwa serwera nie może być pusta.
mp-server-address-title = Adres serwera
mp-server-address-prompt = Wpisz adres host:port.
mp-server-address-duplicate = Ten adres serwera już istnieje.
mp-server-added = Dodano adres serwera.
mp-server-name-updated = Zaktualizowano nazwę serwera.
mp-server-address-updated = Zaktualizowano adres serwera.
mp-server-deleted = Usunięto adres serwera.
mp-server-selected = Wybrano adres serwera.
mp-server-rename = Zmień nazwę
mp-server-edit-address = Edytuj adres
mp-server-delete = Usuń
mp-server-delete-confirm = Usunąć ten serwer multiplayer?

item-adjust = Automatyczne ustawienie opóźnienia
item-adjust-sub = Ustaw opóźnienie automatycznie, aby zsynchronizować muzykę i beatmapę
item-music = Głośność muzyki
item-sfx = Głośność SFX
item-cali = Ustaw opóźnienie

item-dc-pause = Kliknij podwójnie, aby zatrzymać
item-dhint = Podwójna wskazówka
item-dhint-sub = Nuty, które dotykają linii jednocześnie będą podświetlane
item-opt = Agresywna optymalizacja
item-opt-sub = Popraw wydajność, może to spowodować niestabilność
item-use-keyboard = Użyj klawiatury
item-use-keyboard-sub = Włącz wprowadzanie z klawiatury do gry. Wyniki nie mogą być przesłane po włączeniu.
item-prefer-reduced-motion = Preferuj zmniejszony ruch
item-prefer-reduced-motion-sub = Zmniejsza animacje i efekty wizualne
item-note-size = Rozmiar nut

load-cali-failed = Nie załadowano plików audio

item-chart-debug = Tryb debug
item-chart-debug-sub = Pokazuje liczbe linii i orientację
item-touch-debug-sub = Pokazuje dotknięcia

about-content =
  Phira v{ $version }

  Phira to niekomercyjna, oparta na społeczności gra rytmiczna, inspirowana grą Phigros.

  Konto BiliBili: @Phira官方
  Grupa QQ: r48eajexth
  Serwer Discord: discord.gg/gqpR3bTSsP

  Zalecamy dołączenie do grupy QQ lub serwera Discord, aby otrzymywać aktualizacje na żywo i uzyskać pomoc.

  Lista personelu (posortowana leksykograficznie)
  Rozwój
  { $development }

  Operacje
  { $operations }

  Dokumentacja
  { $documentation }

  Sztuka
  { $art }

  Muzyka
  { $music }

  Dźwięk
  { $audio }

  Zarządzanie społecznością
  { $community }

  Lokalizacja
  { $localization }

  I wielu innych dobrowolnych recenzentów chartów. Pełną listę można znaleźć na https://phira.moe/staff

item-render-backend = Render Backend
item-render-backend-sub = Requires restart to take effect
item-render-backend-auto = Auto
item-render-backend-wgpu = Vulkan
item-render-backend-opengl = OpenGL
item-perf-monitor = Monitor wydajności
item-perf-monitor-sub = Pokaż FPS/CPU/Pamięć/GPU/Dysk w lewym górnym rogu podczas gry
