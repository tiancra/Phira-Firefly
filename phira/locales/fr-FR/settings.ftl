...
mp-server-page = Serveurs multijoueur
mp-server-none = Aucun serveur sélectionné
mp-server-current = Actuel
mp-server-name-title = Nom du serveur
mp-server-name-prompt = Saisissez une note pour ce serveur.
mp-server-name-empty = Le nom du serveur ne peut pas être vide.
mp-server-address-title = Adresse du serveur
mp-server-address-prompt = Saisissez une adresse hôte:port.
mp-server-address-duplicate = Cette adresse de serveur existe déjà.
mp-server-added = Adresse du serveur ajoutée.
mp-server-name-updated = Nom du serveur mis à jour.
mp-server-address-updated = Adresse du serveur mise à jour.
mp-server-deleted = Adresse du serveur supprimée.
mp-server-selected = Adresse du serveur sélectionnée.
mp-server-rename = Renommer
mp-server-edit-address = Modifier l'adresse
mp-server-delete = Supprimer
mp-server-delete-confirm = Supprimer ce serveur multijoueur ?
...
about-content =
  ...
  Liste du personnel (triés par ordre alphabétique)
  Développement
  { $development }
...
item-perf-monitor = Moniteur de performances
item-perf-monitor-sub = Afficher FPS/CPU/Mémoire/GPU/Disque en haut à gauche en jeu
